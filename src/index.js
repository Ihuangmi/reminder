(function () {
  ...
  if (!loginTime) {
    ...
    if (
      date.getDate() !== loginD.getDate() ||
      nowTime - loginTime >= 24 * 3600 * 1000
    ) {
      tasksFinished = [];
      tasksTodo = [];
  ...
  document.querySelector(".submit-task").addEventListener("click", () => {
    ...
    if (!!time) {
      ...
      window.electronAPI.setTaskTimer(time, name);
    }
    genTodo();
    activeTab(0);
    activeContent(0);
